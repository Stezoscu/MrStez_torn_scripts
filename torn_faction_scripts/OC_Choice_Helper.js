// ==UserScript==
// @name         Torn OC Best Role Helper (Clean)
// @namespace    ace.torn.oc.ev
// @version      1.1.0
// @description  Shows the single best OC role for you based on EV on the faction crimes tab, with direct crime and item market links
...
(function () {
  ...
  async function fetchCrimes() {
    ...
    try {
      ...
      if (data.error) {
        panel.innerHTML = `API error: ${escapeHtml(data.error.error || 'Unknown error')}`;
        return;
      }

      if (!data.crimes || !Array.isArray(data.crimes) || data.crimes.length === 0) {
        panel.innerHTML = 'No recruiting OCs found.';
        return;
      }

      const { best, consideredCount } = processCrimes(data.crimes);

      if (!best) {
        panel.innerHTML = consideredCount === 0
          ? 'No usable open roles found. This may mean payout data is missing for the available crimes.'
          : 'No open roles found.';
        return;
      }
  ...
  function processCrimes(crimes) {
    ...
    return { best: rows[0] || null, consideredCount };
  }
